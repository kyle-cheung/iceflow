...
use anyhow::Result;
use greytl_cli::commands::compact::{self, Args};
use greytl_cli::commands::run::{self, Args as RunArgs, SinkKind};
...
fn compact_command_rewrites_small_files_and_emits_report() -> Result<()> {
    ...
    assert_eq!(report.snapshot_kind, "greytl-local");
