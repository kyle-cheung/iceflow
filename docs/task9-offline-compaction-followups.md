...
- `cargo test -p iceflow-cli --test compact` uses `crates/iceflow-cli/tests/support/polaris_mock.rs`, which binds `127.0.0.1`. In restricted sandboxes, this suite may require an unsandboxed run even when the code is healthy.

## Deferred Polaris Auth Follow-Up

- `crates/iceflow-cli/src/commands/compact_catalog.rs` currently uses an unauthenticated `reqwest::blocking::Client` for Polaris preflight.
...
- When the CLI contract is widened, add bearer-token and/or client-credentials support to `PolarisCompactionCatalog` using the same auth shape already available on `iceflow-sink::PolarisSink`.

## Deferred Lint Cleanup

- `cargo clippy -p iceflow-cli -- -D warnings` is still blocked by pre-existing `clippy::too_many_arguments` findings in `crates/iceflow-types/src/mutation.rs`.
