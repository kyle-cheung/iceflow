...
pub fn compact_parquet_files(
    ...
    let view_name = format!(
        "compaction_{}_{}",
        sanitize_identifier(output_prefix),
        unique_suffix()
    );
    db.create_parquet_view(&view_name, input_files)?;

    let total_rows = db.count_rows(&view_name)?;
    ...
    while offset < total_rows {
        let file_path = output_dir.join(format!("{output_prefix}-{chunk_index:04}.parquet"));
        let chunk_rows = rows_per_file.min((total_rows - offset) as usize);

        db.copy_view_slice_to_parquet(&view_name, &file_path, row_group_rows, chunk_rows, offset)?;
