...
use anyhow::Result;
use iceflow_sink::{plan_keyed_upsert_commit, CommitRequest, IdempotencyKey, Sink, TestDoubleSink};
...
fn keyed_upsert_latest_wins_and_plans_equality_deletes_plus_appends() -> Result<()> {
    block_on(async {
        ...
        assert_eq!(normalized.records()[0].op, iceflow_types::Operation::Delete);
