...
use iceflow_state::{AttemptResolution, CleanupAction, StateStore, TestStateStore};
use iceflow_types::{
...
fn commit_failure_after_files_written_leaves_batch_recovery_candidates() -> Result<()> {
    block_on(async {
        ...
        let files = manifest
            .file_set
            .iter()
            .map(|file| iceflow_state::BatchFile {
...
fn sample_append_commit_request() -> CommitRequest {
    let source_file_uri = support::sample_source_file_uri("batch-append-0001-recovery");
    CommitRequest {
