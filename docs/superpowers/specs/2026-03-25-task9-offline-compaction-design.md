...
Add a catalog-shaped offline compaction command to `greytl-cli` that rewrites small files for `append_only` tables, emits a machine-readable `CompactReport`, and stays explicitly scoped to the current greytl-local snapshot model rather than pretending to use full Iceberg table semantics.
...
- a private compaction catalog seam inside `greytl-cli`
- one implementation today: `PolarisCompactionCatalog`
- deterministic active-file reconstruction from existing greytl-local snapshot and compaction records
...
- promoting the compaction catalog seam into `greytl-sink` or a new shared crate

## Current Constraint

The current Polaris path is not yet a real Iceberg table integration. `PolarisSink` performs Polaris config and namespace operations and records greytl-local commit state, but it does not create or mutate Iceberg table metadata that a real `LoadTable` call could read. Task 9 therefore compacts against the current greytl-local snapshot model and documents that `CompactReport.snapshot_id` is a greytl-local concept for now.
...
- `greytl-cli::commands::compact` owns a private `CompactionCatalog` trait
...
- load existing greytl-local snapshot and compaction history for the table
...
- verify the target table root already contains greytl snapshot history in the current local model
...
Task 9 compaction runs against the existing greytl-local snapshot model:
...
3. Load existing greytl-local snapshot records for the target table.
4. Load existing compaction records for the target table.
5. Reconstruct the active data-file set deterministically:
   - start from committed files in the base greytl snapshot history
...
Interpret `warehouse_uri` as the current greytl table root for the target adapter.
...
- `snapshot_id` is a greytl-local compaction snapshot id
- records are replayed in lexical filename order

Record payload fields:

- `snapshot_id`
- `snapshot_kind` with value `"greytl-local"`
- `table_mode`
- `namespace`
- `table`
- `warehouse_uri`
...
If the process dies after writing replacement files but before the compaction record lands, those files are treated as orphan files. Task 9 does not invent a new compaction-specific recovery path. Orphan handling stays with the existing reconciliation and orphan-cleanup mechanisms already present in greytl-state.
...
`snapshot_id` is not an Iceberg snapshot id in Task 9. It identifies the greytl-local compaction record that became durable.
...
- a compaction test rewrites multiple small append-only files into fewer files and emits a `CompactReport` with `mode = "offline"` and `snapshot_kind = "greytl-local"`
...
- the CLI command contract is covered through `cargo test -p greytl-cli --test compact`
...
Once that lands, compaction can be upgraded from greytl-local snapshot ids to real Iceberg snapshot semantics without changing the CLI shape introduced in Task 9.
